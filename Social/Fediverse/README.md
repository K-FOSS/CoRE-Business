...
secrets.
Its cache backend is the site-local Dragonfly-compatible Valkey endpoint using
the existing `mastodon-redis` Secret. Because the Tranquil image currently
lacks Redis TLS support, it connects to a pod-local `redis://` socket and the
`socat` sidecar bridges that socket to the Dragonfly TLS endpoint.
Tranquil runs with two replicas because PostgreSQL stores repository state, S3 stores blobs, and
...
and application-specific issuer URL are generated by the Authentik Crossplane
Workspace and consumed from its connection Secret.
No operator PLC recovery key is configured by default; Tranquil therefore
uses its per-account signing keys. If an operator recovery key is added later,
it must be stored as a public `did:key` and supplied through `PLC_ROTATION_KEY`.
The private Tranquil image is pulled from [ATCR](https://atcr.io/) with a
namespace-local Docker registry Secret reconciled from the `AppPassword`
property at `Social/Fediverse/Bluesky` in `mainvault-core`; the registry
username is the non-secret `kjones.pds.mylogin.space` value.
Tranquil uses its native `MAIL_*` configuration with `mail.mylogin.space:465`
and implicit TLS, authenticating with the Tranquil User connection Secret.
